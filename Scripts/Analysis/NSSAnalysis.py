...
IMAGE_W_DEG = 9.99
IMAGE_H_DEG = 7.50
MASK_PPD = IMAGE_WIDTH / IMAGE_W_DEG  # 80.08 pixels/degree for masks
...
# Participants to exclude globally from all analyses
# Use the same IDs as in the `participant` column (ints or strings both ok)
EXCLUDE_SUBJECTS = []   # e.g., [3, 17] or ["P003","P017"]

# Per-session exclusions (drop ONLY the listed session for those participants).
# Keys are "c" (conscious) and "u" (unconscious).
EXCLUDE_SESSIONS = {"c": [], "u": []}   # e.g., {"c":[7, "P015"], "u":[5]}
...
def make_exclusion_mask(df: pd.DataFrame) -> np.ndarray:
    """Return a boolean mask for rows to DROP based on EXCLUDE_SUBJECTS/EXCLUDE_SESSIONS."""
    ...
    if EXCLUDE_SESSIONS and any(EXCLUDE_SESSIONS.values()):
        ...
        for cond_key, plist in EXCLUDE_SESSIONS.items():
            ids = set(map(str, plist))
            if ids:
                m |= (c == str(cond_key).lower()) & p.isin(ids)
    return m
...
def load_fixations(path: Path = FIX_FILE) -> pd.DataFrame:
    ...
    if mask.any():
        ...
        df = df.loc[~mask].copy()
        if DEBUG:
            print(f"Excluded {removed} fixation rows via global/session rules.")
...
def pixels_per_visual_degree(
    ...
    viewing_distance_cm=VIEWING_DIST_CM,
) -> float:

"""Attaching trial, phase and behavioural information to the detected events.

The last stretch of Stage 1, in call order:
  - load_behavioural_from_mat        read the MATLAB expdata struct, matched by row order
  - assign_trial_metadata_and_phases join trial metadata onto events and label each one
                                     disambiguation / mooney / inter_stimulus
  - apply_behavioral_filters_and_save apply the exclusions, derive `awareness`, write the
